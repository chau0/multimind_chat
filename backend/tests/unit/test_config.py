...
class TestSettings:
    ...
    def test_is_using_azure_openai_partial_config(self):
        """Test is_using_azure_openai with partial Azure config."""
        # Only endpoint, no key
        settings1 = Settings(azure_openai_endpoint="https://test.openai.azure.com/")
        assert settings1.is_using_azure_openai is False
        
        # Only key, no endpoint
        settings2 = Settings(azure_openai_api_key="test-key")
    ...
    def test_effective_openai_api_key_standard(self):
        """Test effective_openai_api_key with standard OpenAI."""
        settings = Settings(openai_api_key="sk-standard-key")
        
        result = settings.effective_openai_api_key
        assert result == "sk-standard-key"

    def test_effective_openai_api_key_no_config(self):
    ...
    def test_supabase_url_parsing(self):
