...
from app.repositories import agent_repo, chat_repo
...
async def reset_test_data(db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Failed to reset test data: {str(e)}"
        )
...
async def seed_test_data(db: Session = Depends(get_db)):
    ...
    try:
        ...
        for agent_data in default_agents:
            ...
            existing_agent = db.query(Agent).filter_by(name=agent_data["name"]).first()
            if not existing_agent:
                # Create new agent using SQLAlchemy model directly
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Failed to seed test data: {str(e)}"
        )
...
async def test_status():
    ...
    return {
        ...
        "database_url": (
            os.getenv("DATABASE_URL", "").replace("password", "***")
...
async def get_agents_count(db: Session = Depends(get_db)):
    ...
    agents = agent_repo.get_agents(db)
...
async def get_messages_count(
    session_id: str = "default", db: Session = Depends(get_db)
):
    """Get count of messages for a session."""
    check_test_environment()

    messages = chat_repo.get_messages_by_session(db, session_id)
